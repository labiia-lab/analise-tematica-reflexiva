...
def verificar_citacoes(
    ...
    Use fuzzy=True apenas para triagem de possíveis parafrases. Para cumprir a
    Regra de Ouro da ATR, somente status "verificado" por correspondencia exata
    deve ser aceito como citação válida.
    """
    resultados = {}
    ...
    for extrato in extratos_citados:
        ...
        if resultado["status"] == "nao_encontrado" and fuzzy:
            ...
            for fonte in fontes_norm:
                for trecho, inicio, fim, linha_inicio, linha_fim in _janelas_linhas(fonte["texto"]):
                    ...
                    if melhor_resultado is None or score > melhor_resultado["similaridade"]:
                        melhor_resultado = {
                            "status": "parcial" if score >= threshold else "nao_encontrado",
            ...
            if melhor_resultado and melhor_resultado["status"] == "parcial":
